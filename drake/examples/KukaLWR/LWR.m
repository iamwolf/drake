classdef LWR < TimeSteppingRigidBodyManipulator
  methods
    
    function obj=LWR(urdf,options)
      typecheck(urdf,'char');
    ...
    function obj = compile(obj)
      ...
      if (obj.initialized)
        ...
        if (obj.hands > 0)
          ...
          arm_state_frame = getStateFrame(obj);
          if (length(arm_state_frame.getCoordinateNames()) > length(LWRState(obj).getCoordinateNames()))
            arm_state_frame = replaceFrameNum(arm_state_frame, 1, LWRState(obj));
          else
            arm_state_frame = LWRState(obj);
          end
        end
        ...
        if (obj.hands > 0)
          ...
          input_frame  = replaceFrameNum(input_frame,1,LWRInput(obj));
          % Sub in handstates for each hand
          % TODO: by name?
